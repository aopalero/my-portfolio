...
export function Skills() {
  return (
    <section id="skills" className="bg-gradient-to-br from-[#FF014F]/5 to-[#FF014F]/10 py-16 md:py-20 lg:py-24 scroll-mt-20 md:scroll-mt-24 relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-20">
        <div 
          className="absolute inset-0" 
          style={{
            backgroundImage: `radial-gradient(circle at 30% 20%, rgba(255, 1, 79, 0.1) 0%, transparent 50%), 
                              radial-gradient(circle at 70% 70%, rgba(255, 1, 79, 0.1) 0%, transparent 50%), 
                              radial-gradient(circle at 10% 80%, rgba(255, 1, 79, 0.1) 0%, transparent 50%)`
          }}
        />
      </div>
      ...
      <div className="mx-auto max-w-5xl px-6 text-center relative">
        ...
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-tight">
          The Tools I Use to Build Your<br />
                      <span className="text-[#FF014F]">Future-Ready Web App</span>
        </h2>
        ...
        <div className="mt-12 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6">
          {TECHNOLOGIES.map((tech) => (
            ...
            >
              ...
              <p className="text-xs text-[#FF014F] font-semibold">{tech.category}</p>
                              <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#FF014F]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
            </div>
          ))}
        </div>
            ...
            {TECHNOLOGY_BENEFITS.map((benefit) => (
              <div key={benefit.title}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-2 h-2 bg-[#FF014F] rounded-full"></span>
